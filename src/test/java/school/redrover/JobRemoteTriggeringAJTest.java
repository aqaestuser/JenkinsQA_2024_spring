...
public class JobRemoteTriggeringAJTest extends BaseTest {
    ...
    private void openUserConfigurations() {
        ...
        getWait5().until(ExpectedConditions.visibilityOfElementLocated(By.id("people")))
    ...
    private String[] getTokenUuidUser(String projectName) {
        getWait5().until(ExpectedConditions.elementToBeClickable(By.xpath("//button[text()='Add new Token']")))
                .click();
        ...
        final String token = getWait5().until(ExpectedConditions.visibilityOfElementLocated(
                By.xpath("//span[@class='new-token-value visible']")))
                .getText();
    ...
    private void createFreestyleProjectWithConfigurations(String projectName) {
        ...
        getWait5().until(ExpectedConditions.elementToBeClickable(By.linkText("New Item")))
                .click();
        getWait5().until(ExpectedConditions.presenceOfElementLocated(By.name("name")))
        ...
        getWait2().until(ExpectedConditions.visibilityOfElementLocated(By.name("authToken"))).sendKeys(projectName);

        scrollToElement(By.id("build-environment"));
        getDriver().findElement(
                By.cssSelector("span:has(input[name='hudson-plugins-timestamper-TimestamperBuildWrapper'])")
                )
                .click();
